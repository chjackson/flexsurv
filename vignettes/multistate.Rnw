...
A ``mixture'' multi-state model, first described for competing risks models by \citep{larson1985mixture} is described in terms of: 
...
a individual ``censored" at time $t$ contributed a row in the data \emph{for each} of the events that might have occurred after $t$.   The only difference from \code{bosmx3} is that for these
individuals, \code{bosmx3} only has \emph{one} row per ``censored" individual.  For example, patient 5 is censored at 11 years, when they were still at risk of either BOS or death. 
...
censoring record.  Each row of \code{bosmx3} then corresponds to a transition.   A new column called \code{event} is created, which identifies the terminal event of the transition,  which takes the value \code{NA} in cases of censoring where the transition that will happen is unknown.   Columns not needed for the mixture model are removed.  Labels are added to identify the states. 
...
The \code{flexsurvmix} function fits a mixture model to competing risks data.  To fit a full multi-state mixture model, we fit one mixture competing risks model for each transient state.  The models are then grouped together (Section~\ref{sec:fmixmsm}) to form the full multi-state model.  

The mixture model for the event following transplant (BOS or death before BOS, the 1-2 and 1-3 transitions in the multi-state model) is fitted as follows.  A Weibull distribution is fitted for the time to BOS given BOS onset, and an exponential distribution is fitted for the time to death given death before BOS onset.  These distributions are specified in the \code{dists} argument.  The same distributions as in \code{flexsurvreg} are supported (including custom distributions, in the same way). 
...
The maximum likelihood estimates are in column \code{est}, and estimates on a logit or log
transformed scale are in column \code{est.t} with standard errors on the transformed scale in \code{se}. 
...
The mixture models fit nicely here. Note a \code{start} argument is supplied to give an informative label in the plots to the starting state. 
...
ggplot(aj, aes(x=time, y=val, lty=model, col=state)) + 
  geom_line() +  
  xlab("Years after transplant") + ylab("Probability of being in state")
