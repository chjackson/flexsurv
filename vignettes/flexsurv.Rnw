...
%% mention time dependent covs, \code{survSplit} function in \pkg{survival} 
...
\subsection{Fitting parametric multi-state models using \code{flexsurvreg}}
...
it is essentially non-parametric.  Note that a \code{strata} function
is not currently understood by \code{flexsurvreg} --- the user must be
explicit about what parameters, if any, vary with the transition type,
as in \code{bwei}.
<<>>=
...
@
These can be plotted as follows to show the fit of the parametric
...
\[ \frac{d P(t_0,t)}{dt} = P(t_0,t) Q(t) \]   
with initial condition $P() = I$, as reviewed e.g. by \citet{titman:nonhomog}.
An approximate solution of these is given by a product integral
\[ P(t_0, t) = \prod_{i=0}^{m-1} (I + Q(t_i) du) \]
where a fine grid of times $t_0,t_1,\ldots,t_m=t$ is chosen to span the 
prediction interval, and $Q(t_i) du$ is the increment in the cumulative
hazard matrix between times $t_i$ and $t_{i+1}$ (which may depend on other 
covariates known in ad.   

In \pkg{mstate} these can be specified with the \code{probtrans}
function, applied to the cumulative hazards returned by \code{msfit}.
For the semi-parametric models by \pkg{mstate}, the time grid is based
on the observed survival times, giving the Aalen-Johansen estimator.
For parametric models, using a discrete-time approximation like this
was suggested by \citet{cook:lawless}, and can be made more accurate
by choosing a finer time grid.  An alternative approach would be to
solve the Kolmogorov forward equation numerically, the approach taken
by \citet{titman:nonhomog}.
...
event times, and are not discussed here.

doesn't do Fine and Gray regression, for discussion see, e.g. Putter
et al.
...
\pkg{flexsurv} currently relative survival, used in cancer.  REF 
can this be done with offsets?  see email

Frailty models, \citep{hougaard1995frailty} where multiple survival
times are assumed to be correlated within groups, would be a useful
extension.  See, e.g. \citep{crowther2014multilevel} for a recent
application based on parametric survival models.  This may be done by
exploiting tractability for specific distributions, such as gamma
random effects in Weibull and related models, or by adjusting standard
errors to account for clustering, as implemented in \code{survreg}.
However, implementing substantial model-specific code might conflict
with \pkg{flexsurv}'s aim of generality.  More complex models would
require numerical integration, and \citep{crowther2014multilevel}
...
included in the package as built-in distributions, or at least shown
in a gallery of examples, which already exists as a short vignette.
NAME IT.

behaviour of new classes of models -- initial values, interpreting the
params, sensible defaults
