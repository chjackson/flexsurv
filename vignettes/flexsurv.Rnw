...
through link-transformed linear models $g_0(\mu) = \gamma_0 + \bm{\beta}_0^{\top}
\mathbf{z}$ and $g_r(\alpha_r) = \gamma_r + \bm{\beta}_r^{\top} \mathbf{z}$. $g()$
...
The likelihood for the parameters $\bm{\theta} =
...
(\code{dist = "gompertz"}), the log-logistic, and the three- and four-parameter families
...
\pkg{eha} \footnote{though since version 0.5.1, this distribution is built into \pkg{flexsurv}}   The survivor function is $S(t|\mu,\alpha) = 1/(1 + (t/\mu)^\alpha)$,
...
\[g(S(t,z)) = s(x, \bm{\gamma}) + \bm{\beta}^\top \mathbf{z} \]
...
\[q_{rs}(t,\mathbf{z}(t),\mathcal{F}_t) = \lim_{\delta t \rightarrow 0} \Prob(S(t+\delta t) = s | S(t) = r, t, \mathbf{z}(t), \mathcal{F}_t) / \delta t \] 
