...
\Abstract{ \pkg{flexsurv} is an R package for fully-parametric
  ...
  plotted.  \pkg{flexsurv} also provides functions for predicting
...
in \code{mstate} \citep{mstate:cmpb,mstate:jss} provides
...
@
Confidence intervals can be obtained by simulation from the asymptotic
distribution of the maximum likelihood estimates --- see
\code{help(pmatrix.fs)} for full details.

TODO explain results here instead of in mstate bit
TODO put totlos in.  write totlos markov function.  say msms in development
...
The function \code{sim.fmsm} produces simulated trajectories from
parametric semi-Markov models by simulating the time to the next
...
\code{pmatrix.simfs} wraps \code{sim.fmsm} to calculate a transition
probability matrix.  1000 samples from \code{mssample} give estimates
of transition probabilities that are accurate to within around 0.02,
and similar to the clock-forward estimates.  However with
\code{pmatrix.simfs} we can achieve greater precision by simulating
100 times as many trajectories in a shorter time.  Simulation-based
...
Alternatively, predictions from parametric multi-state
models can be made by supplying the cumulative transition-specific hazards,
...
matrices returned by \code{pmatrix.fs}.  While \code{probtrans} is
slower to achieve a similar level of accuracy to \code{pmatrix.fs} for
the point estimates, an advantage of \code{probtrans} is that standard
errors are available more cheaply.
...
generally less efficient than \code{pmatrix.simfs}.
<<eval=FALSE>>=
...
numerical integration, for example, \citet{crowther2014multilevel} provide
