...
In fact any function of the parameters can be summarised or plotted.
For example, the median of the Weibull can be summaries as follows
<<>>=
...
@ 
The median of the Weibull has an analytic form as $\mu \log(2)^{1/\alpha}$, though
the form of the code given here generalises to other distributions.
...
\code{"BFGS"} method (\citep{nash}) using the analytic derivatives of
the likelihood with respect to the model parameters, if these are
available, to improve the speed of convergence to the maximum.  These
are built-in for the exponential, Weibull and Gompertz.  %% and spline models
