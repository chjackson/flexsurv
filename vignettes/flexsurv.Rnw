...
\Abstract{ \pkg{flexsurv} is an R package for fully-parametric
  ...
  distribution may be fitted if the user supplies a
  probability density or hazard function, and ideally also 
  their cumulative versions.   Standard survival
  distributions are built in, including the three and four-parameter
  generalized gamma and F distributions.  Any parameter of any
  distribution can be modelled as a linear or log-linear function of
  covariates.  The package also includes the spline model of
  \citet{royston:parmar}, in which both baseline survival and
  covariate effects can be arbitrarily flexible parametric functions
  of time.  The main model-fitting function, \code{flexsurvreg}, uses
  the familiar syntax of \code{survreg} from the standard
  \pkg{survival} package.  Censoring or left-truncation are specified
  in \code{Surv} objects.  Estimates and confidence intervals for any
  function of the model parameters can be printed or plotted.
  \pkg{flexsurv} also provides some tools for predicting from 
  fully-parametric multi-state models, and connects with the \pkg{mstate}
  package \citep{mstate:jss}.    This article explains the
...
\S\ref{sec:multistate} the use of \pkg{flexsurv} for parametric
...
having survived up to $s_i$, thus $s_i=0$ if there is no
left-truncation.  Time-dependent covariates (\S\ref{sec:censtrunc})
and multi-state models (\S\ref{sec:multistate}) can be represented
...
This represents time of death or cancer recurrence when \code{censrec}
...
beforehand (\code{plot(...,type="n")}), and use the \code{lines()} methods, 
...
supply derivatives of their custom summary function if possible, so that the 
...
convergence to the maximum.  These derivatives are built-in for the exponential,
Weibull, Gompertz, and hazard- and odds-based spline models
...
\item[\code{name}] Name of the distribution.  In the first example below, 
  we use a logistic distribution, and the name is \code{"llogis"}. 
...
\item[\code{inits}] A function which provides plausible initial values
  ...
  estimate of $\mu$ might be the median log uncensored
  survival time divided by $\log(2)$, with $\alpha=1$, assuming that
  in practice the true value of $\alpha$ is not far from 1\footnote{though currently a different method is used in the package, a simplification of the method in survreg.}.  Then
  we would define the function, of one argument \code{t} assumed to be the
...
for all these models are available as \code{dsurvspline} and \code{psurvspline}.
...
the state of the process at a series of times
...
the exact event times enables much more flexible models, as shown
in this section.
...
first two use the ``clock-reset'' time scale.  The first
...
\Sexpr{round(crexp$AIC)} for the exponential model.  For the 2--3 transition,
...
\caption{Cumulative hazards for three transitions in the BOS multi-state model (clock-reset), under non-parametric, exponential and Weibull models.  For the 2--3 transition, an alternative clock-forward scale is shown for the non-parametric and Weibull models.} 
...
For an time-inhomogeneous Markov model, these are related to the transition
...
@ 
An alternative approach is to solve the Kolmogorov forward equation
numerically, as in \citet{titman:nonhomog}.  This is implemented in
the function \code{pmatrix.fs}, using the \pkg{deSolve} package
\citep{deSolve}.  This returns the full transition probability matrix
$P(t_0,t)$ from time $t_0=0$ to a time or set of times $t$ specified
in the call.  This is more accurate than the time grid approximation 
unless the grid is very fine.  The first row of each matrix below
is close to \code{pstate1}--\code{pstate3} in the corresponding row of \code{ptw}.
<<>>=
pmatrix.fs(cfwei, t=c(5,10), trans=tmat)
@ 
On the other hand, there is no analytic formula for 
confidence intervals, unlike in \code{probtrans}. Therefore these must
be obtained by simulation from the asymptotic distribution of the
maximum likelihood estimates, and by default this is not done.  See
\code{help(pmatrix.fs)} for full details.
...
time $t_0$ and $t$.  $P(t_0, t)$ can then be estimated by
simulation. \pkg{mstate} provides the function \code{mssample} to do
this given the estimated piecewise-constant cumulative hazards
\citep{fiocco:mstatepred}, produced by \code{msfit} or
\code{msfit.flexsurvreg}.  However, \code{flexsurv} provides a more
efficient method for fully-parametric models based on simulating the
exact times to the next transition.  This is fast for built-in distributions
since it calls on the standard vectorised R random number generation 
functions such as \code{rweibull}.  The function \code{sim.fmsm}
produces simulated trajectories this way, and \code{pmatrix.simfs}
wraps this to calculate a transition probability matrix.  1000 samples
from \code{mssample} give estimates of transition probabilities that
are accurate to within around 0.02, and similar to the clock-forward
estimates.  However with \code{pmatrix.simfs} we can achieve greater
precision by simulating 100 times as many trajectories in a shorter
time.  Simulation-based confidence-intervals are also available in
\code{pmatrix.simfs}, at an extra computational cost.
<<eval=FALSE>>=
...
\citep{hougaard1995frailty}, would be a useful extension to \pkg{flexsurv}.  See,
...
numerical integration, and \citet{crowther2014multilevel} provide
...
Because of its aim of generality, \code{flexsurvreg} has not yet been
fine-tuned for specific model classes.  For the Weibull and similar
two-parameter models, \code{survreg} is expected to be more robust and
efficient, as its numerical methods are optimised for those models.

The examples in this paper were run using version 0.4 of \pkg{flexsurv}, which will eventually be available from \url{http://CRAN.R-project.org/package=flexsurv}.  Development versions are available on \url{https://github.com/chjackson/flexsurv-dev}, and contributions are welcome.
