...
\Abstract{ \pkg{flexsurv} is an R package for fully-parametric
  ...
  \pkg{flexsurv} also enhances the \pkg{mstate} package
  \citep{mstate:jss} by providing cumulative hazards for
  fully-parametric multi-state models.  This article explains the
...
For semi-Markov models, the Kolmogorov equation does not apply, since the
transition intensity matrix $Q(t)$ is no longer a deterministic function of $t$, 
but depends on when the transitions occur between time $t_0$ and
...
that are accurate to within around 0.01, and similar to the clock-forward
