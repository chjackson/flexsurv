...
\Abstract{ \pkg{flexsurv} is an R package for fully-parametric
  ...
  probability density or hazard function.  Standard survival
...
no left-truncation.   Time-dependent covariates
(\S\ref{sec:censtrunc}) and multi-state models (\S\ref{sec:multistate}) 
can be represented through left-truncation.

Additionally let $t^{max}_i$ be left-censoring times, a less common
situation.  If there is no left-censoring then these are infinite, so
that $S(t^{max}_i)=0$; or if the $i$th death time is interval-censored
then $c_i=0$ and $t^{max}_i$ is finite; or if there is left-censoring
but no right-censoring then $c_i=0$ and $t_i=0$.
...
or (right-)censoring when \code{censrec} is 0.  The covariate \code{group} is
...
\pkg{stpm2} \citep{stpm2} (historically \pkg{stpm}, \citet{stpm:orig,stpm:update}).
...
package to new classes of models.  New wrappers similar to
\code{flexsurvspline} could be included in the package in the future.
...
and boundary knots are placed at the minimum and maximum (the default knots used by \code{flexsurvspline}). 
...
For illustration, consider a simple three-state example,
...
states are censored at this time. This indicator is coupled with:
\begin{itemize}
...
with BOS has a Weibull distribution.  Any parametric distribution can
be fitted, in principle, just as for standard survival models with
\code{flexsurvreg}. Spline models may also be fitted with 
\code{flexsurvspline}, and are expected to give similar results to the
Cox model.
<<>>=
...
be to solve the Kolmogorov forward equation numerically, as in
\citet{titman:nonhomog}.
