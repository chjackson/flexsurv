...
the maximum likelihood estimates are obtained by \code{survreg} and converted to  \code{flexsurvreg}'s preferred parameterisation.  Therefore the same model can be fitted more directly as
<<>>=
...
(\code{dist = "gompertz"}), log-logistic, and the three- and four-parameter families
...
\pkg{eha} \footnote{though since version 0.5.1, this distribution is built into \pkg{flexsurv}}.   
...
We may want to fit a Weibull model like \code{fs1}, but with the proportional hazards (PH) parameterisation $S(t) =
...
are related to the hazard ratios $\beta$ as $\beta^\prime = -\beta/\alpha$.
...
model with one internal knot, are substantively the same as under a standard Cox model.  
This illustrates that this class of flexible fully-parametric
models may be a reasonable alternative to the semi-parametric Cox model.  
Note the change to \code{scale = "hazard"}, and that a proportional odds
model may fit better than a proportional hazards model (as in this case, see Table \ref{tab:aic})
See \citet{royston:parmar} for discussion of these issues. 
...
@ 
An equivalent of a "stratified" Cox proportional hazards model may be obtained by allowing 
\emph{all} the spline parameters to vary with the categorical covariate that defines the strata. 
In this case, this covariate might be \texttt{group}. 
With \code{k=}$m$ internal knots, the formula should then include \texttt{group}, representing $\gamma_0$, and $m+1$ further terms representing the parameters $\gamma_1,\ldots,\gamma_{m+1}$, named as follows.
<<>>=
...
@ 
would represent a model stratified by group, with a time-varying hazard ratio for treatment.

<<>>=
res <- t(sapply(list(fs1, fs2, fs3, fs4, sp1, sp2, sp3), 
...
depend on patient
characteristics $\mathbf{z}(t)$, the time $t$ itself, and possibly also the ``history'' of
...
After forming survival data as described above, a multi-state model can be
fitted by maximising the standard survival model likelihood~(\ref{eq:lik}), 
$l(\bm{\theta} | \mathbf{x}) = \prod_i l_i(\bm{\theta}|x_i)$,  where $\mathbf{x}$ is the
data, and $i$ now indexes multiple observations for multiple individuals.   
This can also be written as a product over the $K=\sum_r n_r$
transitions $k$, and the $m_k$ observations $j$ pertaining to the $k$th
transition. 
...
likelihood~(\ref{eq:lik:multistate}) with a single call.  The transition type 
will typically enter this model as a categorical covariate.
