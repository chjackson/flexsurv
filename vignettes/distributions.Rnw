...
\end{itemize}
  ...
  measure presented is the hazard ratio (HR) for one unit of the
  covariate.   
...
\code{dgamma(..., shape=a, rate=mu)}

\[
  S(t | a, \mu) = 1 - \int_{0}^t 
  \frac{x^{a-1} \mu^a \exp{-(\mu x)}}{\Gamma(a)}
    dx, \qquad \mu>0, a>0
\]
...
so that higher values of the location parameter $\gamma_0$ lead to higher hazards, or lower survival, for all three choices of link function.  For \verb+scale="hazard"+, $\exp(\bbeta)$ is the hazard ratio, and for \verb+scale="odds"+, $\exp(\bbeta)$ is an odds ratio. 
