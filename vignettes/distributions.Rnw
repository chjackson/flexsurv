...
\end{itemize}
  ...
  alternative way of presenting effects from an accelarated failure
...
non-zero probability as $t$ increases. 
...
\]
where $s()$ is a natural cubic spline function with $m$ internal
knots: 
\[
...
\[
g(S(t,\mathbf{z})) =  \begin{array}{ll}
\log(-\log(S(t,\mathbf{z}))) &\verb+(scale="hazard")+ \\
\log(S(t,\mathbf{z})^{-1} - 1)      &                  \verb+(scale="odds")+ \\
\Phi^{-1}(S(t,\mathbf{z})) &     \verb+(scale="normal")+ \\
                      \end{array}
                    \]
