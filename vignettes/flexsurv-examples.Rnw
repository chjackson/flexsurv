...
\subsection{Prognostic model for the breast cancer data}
...
transformations of the covariates are constructed. \code{progc} is the Cox model used by
...
(progc <- coxph(Surv(time, cens) ~ horTh + X1a + X1b + X4 + X5 + X6, data=GBSG2))
(prog3 <- flexsurvspline(Surv(time, cens) ~ horTh + X1a + X1b + X4 + X5 + X6, k=3, data=GBSG2))
@
