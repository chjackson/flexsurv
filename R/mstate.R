...
sim.fmsm <- function(x, trans=NULL, t, newdata=NULL, start=1, M=10, tvar="trans", tcovs=NULL, tidy=FALSE, debug=FALSE){
    ...
    basepars.all <- flexsurv:::pars.fmsm(x = x, trans = trans, newdata = newdata, 
        tvar = tvar)
       nst <- nrow(trans) # number of states
    res.st <- cur.st <- start # current and final states
     res.t <- cur.t  <- rep(0, M) # current and final transition times
      todo <- seq_len(M) # indices of active individuals (in transient state)
    # loop over active individuals and determine next time/state
    while (any(todo)) {
        # last observation carried forward
        cur.st.out <- cur.st[todo]
         cur.t.out <- cur.t[todo]
              done <- numeric()
        # loop over originating states 
        for (i in unique(cur.st[todo])) {
            if (i %in% flexsurv:::transient(trans)) { # only need to do anything if transient
                  transi <- na.omit(trans[i, ])
                # determine indices of individuals who are still transient AND in state i
                    inds <- which(cur.st[todo] == i)
                    ni   <- length(inds)
                t.trans1 <- matrix(0, nrow = ni, ncol = length(transi))
                for (j in seq_along(transi)) { # simulate times for all competing events
                    xbase <- if (flexsurv:::is.flexsurvlist(x)) 
                      x[[transi[j]]]
                    else x
                    if (length(tcovs) > 0) {
                      basepars <- flexsurv:::form.basepars.tcovs(x, transi[j], 
        ...
        # append as column to res
        res.st <- cbind(res.st, cur.st)
         res.t <- cbind(res.t, cur.t)
        # check for arrival in absorbing state
        done <- union(done, which(cur.st %in% flexsurv:::absorbing(trans)))
